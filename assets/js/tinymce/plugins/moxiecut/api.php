...
$globaldirs = array(
    ...
    "news"=>array("ext"=>"jpg,gif,png,bmp"),
    "pages"=>array("ext"=>"jpg,gif,png,bmp")
);
...
function sendparamsfile($path,$size,$modify,$file=false){
    global $url;
    $data = array(      //äëÿ òîãî ÷òîáû àâòîìàòîì ïîäêëþ÷àëî ðåæèì thumbnails
        "path"=>$path,
        "size"=>$size,
        "lastModified"=>$modify,
        "isFile"=>$file,
...
if($_SERVER["REQUEST_METHOD"] == "POST")
{
    ...
    //èíèöèàëèçàöèÿ, âûâîä ïàïîê, êîíôèãè äëÿ íèõ
    if($method == "listRoots")
    {
        $OUT["jsonrpc"] = $jsrpc;
        $OUT = refreshdirs();
    ...
    {
        $OUT["jsonrpc"] = $jsrpc;
        if($PARAMS["only_files"])   //ïðîñìîòð èçîáðàæåíèé
        {
    ...
    if($method == "getConfig")  // àïëîä ôàéëîâ
    {
        $OUT["jsonrpc"] = $jsrpc;
    ...
    if($_GET["action"] == "upload") //çàêà÷êà ôàéëîâ
    {
        $OUT["jsonrpc"] = $jsrpc;
    ...
    if($method == "delete")  // óäàëåíèå ôàéëîâ
    {
        $OUT["jsonrpc"] = $jsrpc;
    ...
    if($method == "createDirectory")  // ñîçäàíèå äèðåêòîðèè
    {
        mkdir($workdir.$path);
        $OUT["jsonrpc"] = $jsrpc;
        $OUT = refreshdirs();
    ...
    if($method == "moveTo")  // ïåðåèìåíîâàíèå, ïåðåíîñ
    {
        $OUT["jsonrpc"] = $jsrpc;
        $from = $PARAMS["from"];
        ...
        else
        {
            ...
            if(!strstr($from,"..") && !strstr($to,"..") && !in_array($type,$forbidden))
            {
                ...
                rename($workdir.$from,$to);  //ïåðåèìåíîâàíèå
            }
            else
               $OUT["error"] = array("code"=>"100","message"=>"Không có quyền truy cập file này! Ðổi tên file bị hủy bỏ!","data"=>"");
        }
        $OUT = refreshdirs();
        $OUT["id"] = $id;
    }

    if($method == "copyTo")  // êîïèðîâàíèå
    {
        $from = $PARAMS["from"];
        $to = iconv('utf-8', $charset, $workdir.$PARAMS["to"]);
        foreach($from as $file)
